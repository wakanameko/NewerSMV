...
bool dakoton_c::collisionCat13_Hammer(ActivePhysics *apThis, ActivePhysics *apOther) {
	...
	DisplayScoreAddPopup(thispos, NORMAL_200, apOther->owner->which_player, false);
...
int dakoton_c::onCreate() {
	...
	// Reggieで設定できるようにするやつ		// Reggie! configs
...
void dakoton_c::executeState_jumpkoton(){
	// jumping
	//if (this->timer < 30){
	if (this->jumphight != 0){	// branch when this->jumphight > 0
