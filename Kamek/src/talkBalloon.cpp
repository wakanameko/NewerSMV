...
dActor_c *dTalkBalloon_c::build() {
	...
	return new(buffer) dTalkBalloon_c;
...
Profile talkBalloonProfile(&dTalkBalloon_c::build, SpriteId::TALKBALLOON, &talkBalloonData, ProfileId::TALKBALLOON, ProfileId::TALKBALLOON, "TALKBALLOON", NULL, 0);
...
void dTalkBalloon_c::showTB(int category, int textID, Vec3 pos) {
	if(!isShown) {
		isShown = true;
	...
	OSReport("showTB called.\n");

		WriteBMGToTextBox(T_text[0], GetBMG(), category,textID, 0);
		WriteBMGToTextBox(T_text[1], GetBMG(), category, textID, 0);
		T_text[0]->DrawSelf(layout.drawInfo);

	OSReport("showTB called.\n");

		bool twoLines = getLineCount(GetBMGMessage(category, textID));
