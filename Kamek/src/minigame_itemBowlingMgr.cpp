#include "minigame_itemBowling.h"
#include "boss.h"
...
dActor_c *dItemBowlingMgr_c::build() {
...
const SpriteData minigameItemBowlingMgrData = {ProfileId::ITEMBOWLINGMGR, 0, 0, 0, 0, 0x1000, 0x1000, 0, 0, 0, 0, 2}; 
Profile minigameItemBowlingMgrProfile(&dItemBowlingMgr_c::build, SpriteId::ITEMBOWLINGMGR, &minigameItemBowlingMgrData, ProfileId::ITEMBOWLINGMGR, ProfileId::ITEMBOWLINGMGR, "ITEMBOWLINGMGR", NULL, 0);
...
u8 itemToInventory[8] = {0, 1, 2, 4, 5, 3, 6, 7};	// Added Hammer element(7)
...
void dItemBowlingMgr_c::showWin() {
	...
	for(int i = 0; i < 8; i++) {
		if(wonItemsCount[i] > 0) {
			...
			if (save->powerups_available[itemToInventory[i]] > 99){
				save->powerups_available[itemToInventory[i]] = 99;
			}
