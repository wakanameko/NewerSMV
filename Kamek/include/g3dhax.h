...
class mHeapAllocator_c : public mAllocator_c {
	public:
		...
		// below 2 rows from NSMBW-TPC (by Asu-chan)
...
namespace m3d {
	...
	class scnLeaf_c {
		public:
			...
			void setScale(Vec *vec);	// from NSMBW-TPC (by Asu-chan)
...
// class bmdl_c from NSMBW-TPC (by Asu-chan)
class bmdl_c : public scnLeaf_c {
	...
	class banm_c {
		public:
			...
			//virtual int _vf0C() = 0;
			virtual int _vf0C();	//from NSMBW-TPC (by Asu-chan)
			virtual void detach();
	...
	// class anmChrPart_c from NSMBW-TPC (by Asu-chan)
